...
import typer
from ai import AI
from config import EXCLUDED_EXTENSIONS_SOURCE_PATTERNS
...
def main(
    ...
    sourcedir: str = typer.Option(
        "../benchmarks/flask-nodejs/source",
        help="Source directory containing the code to be migrated.",
    ),
    sourcelang: str = typer.Option(
        None, help="Source language or framework of the code to be migrated."
    ),
    ...
    targetdir: str = typer.Option(
        "../benchmarks/flask-nodejs/target",
        help="Directory where the migrated code will live.",
    ),
    targetlang: str = typer.Option(
        "nodejs", help="Target language or framework for migration."
    ),
    operating_system: str = typer.Option(
        "linux",
        help="Operating system for the Dockerfile. Common options are 'linux' or 'windows'.",
    ),
    ...
    step: str = typer.Option(
        "all", help="Step to run. Options are 'setup', 'migrate', 'test', 'all'."
    ),
):
    ai = AI(
        model=model,
    ...
    if not sourcelang:
        if detected_language:
            is_correct = typer.confirm(
                f"Is your source project a {detected_language} project?"
            )
            if is_correct:
                sourcelang = detected_language
            else:
                sourcelang = typer.prompt(
                    "Please enter the correct language for the source project"
                )
        else:
            sourcelang = typer.prompt(
                "Unable to detect the language of the source project. Please enter it manually"
            )
    ...
    source_directory_structure = build_directory_structure(
        sourcedir, ignore_patterns=EXCLUDED_EXTENSIONS_SOURCE_PATTERNS
    )
    globals = Globals(
        sourcedir,
        targetdir,
    ...
    if step in ["migrate", "all"]:
        ...
        def migrate(sourcefile, globals, parent_file=None):
            ...
            internal_deps_list, external_deps_list = get_dependencies(
                sourcefile=sourcefile, globals=globals
            )
    ...
    typer.echo(
        typer.style("All tests complete. Ready to rumble. 💪", fg=typer.colors.GREEN)
    )
