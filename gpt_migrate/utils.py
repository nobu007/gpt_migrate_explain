...
def prompt_constructor(*args):
    prompt = ""
    for arg in args:
        with open(os.path.abspath(f"resource/prompts/{arg}"), "r") as file:
...
def llm_write_file(
    prompt, target_path, waiting_message, success_message, globals, targetdir=None
):
    file_content = ""
    with yaspin(text=waiting_message, spinner="dots") as spinner:
    ...
    if target_path:
        target_path_final = os.path.join(targetdir, target_path)
    else:
        target_path_final = os.path.join(targetdir, file_name)
    os.makedirs(os.path.dirname(target_path_final), exist_ok=True)
    with open(target_path_final, "w", encoding="utf-8") as file:
...
def write_file_explain(explainfile_relative_path, globals):
    explainfile_content = globals.explainfile_content

    explainfile_path = os.path.join(globals.explaindir, explainfile_relative_path)
    os.makedirs(os.path.dirname(explainfile_path), exist_ok=True)
    with open(explainfile_path, "w", encoding="utf-8") as file:
        file.write(explainfile_content)

    return explainfile_path
...
def build_directory_structure(
    ...
    if os.path.isdir(path):
        ...
        for index, entry in enumerate(entries):
            ...
            if not is_ignored(entry_path, ignore_patterns):
...
def file_exists_in_memory(filename):
    file = Path("gpt_migrate_explain/gpt_migrate/memory/" + filename)
    return file.exists()
...
def write_to_memory(filename, content):
    with open("gpt_migrate_explain/gpt_migrate/memory/" + filename, "a+") as file:
        existing_content = file.read().split("\n")
        for item in content:
...
def read_from_memory(filename):
    content = ""
    with open("gpt_migrate_explain/gpt_migrate/memory/" + filename, "r") as file:
        content = file.read()
    return content
...
def extract_json_from_response(response_content: str) -> str:
    ...
    print("extract_json_from_response response_content=", response_content)
    return response_content


def extract_dict_from_response(response_content: str) -> dict[str, Any]:
    # Sometimes the response includes the JSON in a code block with ```
    response_content = extract_json_from_response(response_content)
...
def escape_brakect(text: str) -> str:
    """escape_brakect"""
    return text.replace("{", "{{").replace("}", "}}")


def unescape_brakect(text: str) -> str:
    """unescape_brakect"""
    return text.replace("{{", "{").replace("}}", "}")


def get_near_source_directory_structure(
    source_directory_structure: str, sourcefile: str
) -> str:
    """get_near_source_directory_structure"""
    near_source_directory_structure = []
    lines = source_directory_structure.split("\n")
    for i, line in enumerate(lines):
        if sourcefile in line:
            start_index = max(0, i - 5)
            end_index = min(len(lines), i + 6)
            near_source_directory_structure.extend(lines[start_index:end_index])
    return "\n".join(near_source_directory_structure)
