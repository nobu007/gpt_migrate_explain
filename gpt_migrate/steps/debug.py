...
def debug_error(error_message, relevant_files, globals):
    identify_action_template = prompt_constructor(
        HIERARCHY, GUIDELINES, IDENTIFY_ACTION
    )

    prompt = identify_action_template.format(
        error_message=error_message[
            -min(MAX_ERROR_MESSAGE_CHARACTERS, len(error_message)) :
        ],
    ...
    if "MOVE_FILES" in action_list:
        ...
        move_files_template = prompt_constructor(
            HIERARCHY, GUIDELINES, WRITE_CODE, MOVE_FILES, SINGLEFILE
        )
        ...
        if typer.confirm(
            "GPT-Migrate wants to run this shell script to debug your application, which is also stored in gpt_migrate/debug.sh: \n\n"
            ...
            try:
                ...
                error_text = typer.style(
                    "Something isn't right with your shell script. Please ensure it is valid and try again.",
                    fg=typer.colors.RED,
                )
    ...
    if "EDIT_FILES" in action_list:
        if relevant_files != "":
            ...
            for file_name in fileslist:
                ...
                files_to_construct.append(
                    ("migration_source/" + file_name, file_content)
                )

            relevant_files = construct_relevant_files(files_to_construct)

        identify_file_template = prompt_constructor(
            HIERARCHY, GUIDELINES, IDENTIFY_FILE, FILENAMES
        )
        ...
        prompt = (
            identify_file_template.format(
                error_message=error_message[
                    -min(MAX_ERROR_MESSAGE_CHARACTERS, len(error_message)) :
                ],
                target_directory_structure=build_directory_structure(globals.targetdir),
                docker_logs=docker_logs[
                    -min(MAX_DOCKER_LOG_CHARACTERS, len(docker_logs)) :
                ],
            ),
        )
        ...
        for file_name in file_name_list:
            ...
            try:
                ...
                print(
                    "File not found: "
                    + file_name
                    + ". Please ensure the file exists and try again. You can resume the debugging process with the `--step test` flag."
                )
                raise typer.Exit()

            debug_file_template = prompt_constructor(
                HIERARCHY, GUIDELINES, WRITE_CODE, DEBUG_FILE, SINGLEFILE
            )

            prompt = (
                debug_file_template.format(
                    error_message=error_message[
                        -min(MAX_ERROR_MESSAGE_CHARACTERS, len(error_message)) :
                    ],
                    file_name=file_name,
    ...
    if "CREATE_FILE" in action_list:
        create_file_template = prompt_constructor(
            HIERARCHY, GUIDELINES, WRITE_CODE, CREATE_FILE, SINGLEFILE
        )
        ...
        success_text = typer.style(
            f"Created new file {new_file_name}.", fg=typer.colors.GREEN
        )
...
def debug_testfile(error_message, testfile, globals):
    ...
    relevant_files = construct_relevant_files(
        [("migration_source/" + testfile, source_file_content)]
    )
    ...
    debug_file_template = prompt_constructor(
        HIERARCHY, GUIDELINES, WRITE_CODE, DEBUG_TESTFILE, SINGLEFILE
    )
...
def require_human_intervention(error_message, relevant_files, globals):
    human_intervention_template = prompt_constructor(
        HIERARCHY, GUIDELINES, WRITE_CODE, HUMAN_INTERVENTION, SINGLEFILE
    )
