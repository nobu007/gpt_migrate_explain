    ...
    try:
        ...
        if typer.confirm("Would you like GPT-Migrate to try to fix this?"):
            ...
            require_human_intervention(
                error_message,
                relevant_files=construct_relevant_files([("Dockerfile", dockerfile_content)]),
                globals=globals,
            )
            raise typer.Exit()
...
def create_tests(testfile, globals):
    ...
    prompt = create_tests_template.format(
        targetport=globals.targetport, old_file_content=old_file_content, guidelines=globals.guidelines
    )
...
def validate_tests(testfile, globals):
    try:
        ...
        if typer.confirm("Would you like GPT-Migrate to try to fix this?"):
            ...
            require_human_intervention(
                error_message,
                relevant_files=construct_relevant_files([(f"gpt_migrate/{testfile}", tests_content)]),
                globals=globals,
            )
            raise typer.Exit()
